...
public class CarPlayerAIScript : MonoBehaviour
{
    GameObject playerT;
    GameObject[] playerTs;
    Transform targetT;
    ...
    private void Update()
    {
        ...
        playerTs = GameObject.FindGameObjectsWithTag("Bot");
        if (playerTs.Length > 0) { playerT = playerTs[0];
            ...
            if (distanceToTarget > reachedTargetDistance)// Si la distancia es mes gran que la distancia objectiu. Cotxe va cap a objectiu
            {
                ...
                // Calculs angles, si son massa justos el cotxe fara zig zag. ARA VAN BE
